...
const fetchStats = async () => {
  try {
    ...
    let txQuery = client.from('transactions').select('amount, paid_amount, type, created_at')
    ...
    const totalVolume = txRes.data?.filter(t => t.type === 'deposit').reduce((acc, curr) => acc + Number(curr.paid_amount || 0), 0) || 0
